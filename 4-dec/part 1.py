...
for guard, minutes in sleep.items():
    if sum(minutes) > max_mins:
        max_mins = sum(minutes)
        max_guard = guard

max_guard_schedule = sleep[max_guard]
print(max_guard_schedule, sum(max_guard_schedule))
print(max_guard_schedule.index(max(sleep[max_guard])) * max_guard)
